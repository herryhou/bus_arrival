 ...
 *
 * Parses route_data.bin files generated by preprocessor (v8.5 format).
 * Binary format must match Rust RouteData struct in shared/src/binfile.rs.
 *
 * RouteNode layout (#[repr(C)], 40 bytes):
 * offset  0: len2_cm2     i64   8 bytes
 * offset  8: heading_cdeg i16   2 bytes
 * offset 10: _pad         i16   2 bytes
 * offset 12: x_cm         i32   4 bytes
 * offset 16: y_cm         i32   4 bytes
 * offset 20: cum_dist_cm  i32   4 bytes
 * offset 24: dx_cm        i32   4 bytes
 * offset 28: dy_cm        i32   4 bytes
 * offset 32: seg_len_cm   i32   4 bytes
 * offset 36: _end_pad     i32   4 bytes (alignment padding to 8-byte boundary)
 *
 * Note: v8.5 changed from repr(C, packed) to repr(C) to fix UB with field references.
 * Size increased from 36 to 40 bytes due to 8-byte alignment padding at end.
 *
 ...
 * - Nodes array: node_count × 40 bytes
...
const ROUTE_NODE_SIZE = 40; // v8.5: changed from 36 to 40 (repr(C) alignment padding)
const STOP_SIZE = 12;
const HEADER_SIZE_V8 = 28; // + lat_avg_deg(8) for total of 28 bytes
const MAGIC = 0x42555341; // "BUSA" in little-endian
const VERSION = 3; // v8.5 format: repr(C) with 40-byte nodes
...
/**
 * Parse RouteNode from binary data (v8.5 format - 40 bytes)
 */
function parseRouteNode(dataView: DataView, offset: number): RouteNode {
	return {
		len2_cm2: readI64(dataView, offset),
		heading_cdeg: dataView.getInt16(offset + 8, true),
		_pad: dataView.getInt16(offset + 10, true),
		x_cm: dataView.getInt32(offset + 12, true),
		y_cm: dataView.getInt32(offset + 16, true),
		cum_dist_cm: dataView.getInt32(offset + 20, true),
		dx_cm: dataView.getInt32(offset + 24, true),
		dy_cm: dataView.getInt32(offset + 28, true),
		seg_len_cm: dataView.getInt32(offset + 32, true)
		// Note: offset 36-39 is _end_pad for 8-byte alignment, not used
	};
}
...
/**
 * Parse route_data.bin file from ArrayBuffer (v8.5 format)
 *
...
export function parseRouteData(buffer: ArrayBuffer): RouteData {
	...
	offset += 2;
	if (version !== VERSION) {
		throw new Error(`Unsupported version: ${version}, expected ${VERSION} (v8.5 format). Please regenerate route_data.bin with the latest preprocessor.`);
	}
