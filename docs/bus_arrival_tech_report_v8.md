...
根據我們的討論與技術設計報告（v8.3）的要求，為了確保 **1D 線性距離的準確性** 並解決 **RDP 簡化導致的站點順序反轉** 問題，更新後的預處理流程將「空間格網索引」提前，並引入「路徑感知（Path-Aware）」的站點投影算法。

以下是完整的更新版 **「17. 離線預處理流程」**：
...
| **3** | **段長約束插值** | 檢查簡化後路段，若長度 $> 3000\text{ cm}$（30m），則在中間插入補充節點，確保投影平滑度。 |
...
| **6** | **建立空間格網索引** | **(提前執行)** 根據線性化後的節點建立 $100\text{m} \times 100\text{m}$ 的 Spatial Grid Index。這將用於加速後續的站點投影。 |
| **7** | **序列約束站點投影** | **(核心更新)** 依據輸入站點順序進行投影，採用**兩階段演算法**：<br><br>**階段 A：格網輔助投影（Grid-Assisted Projection）**<br>- 利用 $100\text{m} \times 100\text{m}$ Spatial Grid 進行 $O(k)$ 局部路段搜尋<br>- 漸進式視窗擴展：3×3 → 5×5 → 7×7 cells<br>- 若格網搜尋無結果，回退至線性搜尋<br><br>**階段 B：路徑感知排序（Path-Aware Order）**<br>- **路徑約束：** 從階段 A 的候選路段中，僅選擇索引 $\ge$ `上一站點的路段索引`<br>- **單調性驗證：** 確保 $s_{stop, j} > s_{stop, j-1}$（輸入順序）<br>- **若無有效候選：** 擴大搜尋範圍或執行線性搜尋（從 `min_segment_idx` 到路線末尾）<br><br>**處理重複位置站點（如迴圈路線）：**<br>當多個站點位於相同座標時（例如 #21 和 #23 在同一位置），演算法會：<br>1. 站點 #21：格網找到路段 [498, 499, 500, 501] → 選擇最近者 (500)<br>2. 站點 #22：格網找到路段 [548, 549, 550, 551] → 選擇最近者 (550)<br>3. 站點 #23：格網找到 [498, 499, 500, 501] + [698, 699, 700, 701] → 過濾 $\ge$ 550 → [698, 699, 700, 701]<br>4. **選擇最近者：** 從過濾後的候選中選擇幾何距離最近者 (如 700)<br><br>**反轉檢測處理：**<br>若驗證失敗（進度值反轉），直接報告錯誤並提示：<br>- 檢查 `stops.json` 順序是否與路線幾何相符<br>- 若路線有合法回溯（如迴圈路線），使用 `--skip-validation` 旗標跳過驗證 |
...
1.  **兩階段站點投影演算法：**<br>   - **階段 A - 格網輔助投影：** 利用 $100\text{m} \times 100\text{m}$ Spatial Grid 快速定位站點附近的「候選路段集合」<br>   - **階段 B - 路徑感知排序：** 從候選集合中選擇符合單調性約束的路段（索引 $\ge$ 前一站點）<br>   - **漸進式擴展機制：** 3×3 → 5×5 → 7×7 → 線性搜尋，確保在迴圈路線或重複位置站點也能找到有效路段
2.  **處理重複位置站點：** 對於位於相同座標的多個站點（如迴圈路線的 #21 和 #23），格網自然包含多次經過的路段，階段 B 過濾出後續路段（第二次經過）
3.  **簡化設計：** 移除自動重試機制（epsilon 降低），驗證失敗直接報告錯誤。**設計原則：** 簡化優先於自動修復。若路線幾何導致反轉，通常表示：
      - 輸入站點順序與路線幾何不符
      - 路線有合法回溯（使用 `--skip-validation`）
      - RDP 簡化參數需要手動調整
4.  **V8.3 空間優化：** 嚴格執行 `RouteNode` 結構體瘦身，移除冗餘的線性方程係數，將 Flash 佔用壓低至約 **24 KB**（以 600 節點計算）。
5.  **一致性校驗：** 強調 `lat_avg` 在線性化與站點投影中必須保持絕對一致，並封裝於 binary header 中供 Runtime 直接讀取，消除因緯度縮放引起的位移誤差。
...
6. **序列約束站點投影（Sequence-Constrained Stop Projection）**
   ...
   - `--skip-validation` 旗標：支援跳過驗證（用於合法迴圈路線）
