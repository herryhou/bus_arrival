...
fn scenario_gps_jump_recovery() {
    ...
    let recovered = detection::recovery::find_stop_index(jump_progress, 1000, dt_since_last_fix, &stops, last_index, &None);
    ...
    let recovered = detection::recovery::find_stop_index(jump_progress, 1000, dt_since_last_fix, &stops, 1, &None);
    ...
    let recovered = detection::recovery::find_stop_index(49900, 1000, 1, &stops, 2, &None);
    assert_eq!(recovered, Some(1));
}
