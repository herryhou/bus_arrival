 ...
 *
 * Parses route_data.bin files generated by preprocessor (v8.3 format).
 * Binary format must match Rust RouteData struct in shared/src/binfile.rs.
 *
 * RouteNode layout (#[repr(C, packed)], 36 bytes):
 ...
 *
 * Note: line_a, line_b, line_c removed in v8.3 (16 bytes saved per node)
 *
 ...
 * - Nodes array: node_count × 36 bytes
...
/**
 * Parse RouteNode from binary data (v8.3 format - 36 bytes)
 */
...
/**
 * Parse route_data.bin file from ArrayBuffer (v8.3 format)
 *
...
export function parseRouteData(buffer: ArrayBuffer): RouteData {
	...
	offset += 2;
	if (version !== VERSION) {
		throw new Error(`Unsupported version: ${version}, expected ${VERSION} (v8.3 format)`);
	}
