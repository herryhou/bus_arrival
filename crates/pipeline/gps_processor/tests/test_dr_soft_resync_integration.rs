...
fn test_velocity_soft_resync_formula_verification() {
    ...
    assert!(
        v_full_kalman > expected,
        "Full Kalman gain is more aggressive"
    );
}
...
fn test_recovery_flag_state_transitions() {
    ...
    assert!(
        !dr_normal.in_recovery,
        "Initial state should not be in recovery"
    );
    ...
    assert!(
        !dr_recovered.in_recovery,
        "Recovery flag cleared after soft-resync"
    );
}
...
fn test_conservative_vs_full_kalman_comparison() {
    ...
    assert!(
        s_conservative >= s_kalman_default,
        "Conservative ≈ Kalman default"
    );
    assert!(s_conservative < s_kalman_max, "Conservative < Kalman max");

    println!(
        "Conservative: {}, Kalman min: {}, default: {}, max: {}",
