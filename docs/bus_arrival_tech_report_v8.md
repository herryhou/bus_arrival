...
| **7** | **序列約束站點投影** | **(核心更新)** 依據輸入站點順序進行投影，採用**兩階段演算法**：<br><br>**階段 A：格網輔助投影（Grid-Assisted Projection）**<br>- 利用 $100\text{m} \times 100\text{m}$ Spatial Grid 進行 $O(k)$ 局部路段搜尋<br>- 漸進式視窗擴展：3×3 → 5×5 → 7×7 cells<br>- 若格網搜尋無結果，回退至線性搜尋<br><br>**階段 B：路徑感知排序（Path-Aware Order）**<br>- **路徑約束：** 從階段 A 的候選路段中，僅選擇索引 $\ge$ `上一站點的路段索引`<br>- **單調性驗證：** 確保 $s_{stop, j} > s_{stop, j-1}$（輸入順序）<br>- **若無有效候選：** 擴大搜尋範圍或執行線性搜尋（從 `min_segment_idx` 到路線末尾）<br><br>**處理重複位置站點（如迴圈路線）：**<br>當多個站點位於相同座標時（例如 #21 和 #23 在同一位置），演算法會：<br>1. 站點 #21：格網找到路段 [498, 499, 500, 501] → 選擇最近者 (500)<br>2. 站點 #22：格網找到路段 [548, 549, 550, 551] → 選擇最近者 (550)<br>3. 站點 #23：格網找到 [498, 499, 500, 501] → 過濾 $\ge$ 550 → **局部無候選**<br>4. **自動擴大搜尋：** 擴展至 5×5、7×7 cells，或執行線性搜尋從索引 550 開始<br><br>**反轉檢測與自動修正：**<br>若最終仍導致進度值反轉（$s_j \le s_{j-1}$），則：<br>- 自動降低 RDP $\varepsilon$（700 → 350 → 175 cm，最多 3 次）<br>- 重新簡化受影響區域並重試投影<br>- 若 3 次重試後仍失敗，報告錯誤並提示檢查 `stops.json` 順序 |
...
2.  **處理重複位置站點：** 對於位於相同座標的多個站點（如迴圈路線的 #21 和 #23），演算法會自動擴大搜尋範圍或執行線性搜尋，確保後續站點映射到正確的路段（而非重複使用相同的幾何位置）
3.  **V8.3 空間優化：** 嚴格執行 `RouteNode` 結構體瘦身，移除冗餘的線性方程係數，將 Flash 佔用壓低至約 **24 KB**（以 600 節點計算）。
4.  **一致性校驗：** 強調 `lat_avg` 在線性化與站點投影中必須保持絕對一致，並封裝於 binary header 中供 Runtime 直接讀取，消除因緯度縮放引起的位移誤差。
