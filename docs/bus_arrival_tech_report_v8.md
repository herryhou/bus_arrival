...
| **7** | **序列約束站點投影** | **(核心更新)** 依據輸入站點順序進行投影：<br>1. 利用格網進行 $O(k)$ 局部路段搜尋。<br>2. **路徑約束：** 搜尋起點必須從上一個站點匹配的節點索引開始。<br>3. **單調性驗證：** 確保 $s_{stop, j} > s_{stop, j-1}$。若發生投影反轉（通常因 RDP 縮短曲線引起），自動回溯並降低該區域 RDP 容差重新計算。 |
...
1.  **格網輔助投影 (Grid-Assisted Projection)：** 將格網建立移至站點投影之前，使預處理器能利用 $100\text{m} \times 100\text{m}$ 的索引快速定位候選路段，而非全線盲搜。
2.  **路徑順序優先 (Path-Aware Order)：** 站點投影不再僅依賴「最近距離」，而是強制遵循 `stops.json` 的輸入順序。若幾何投影導致順序錯誤，預處理器會主動介入調整（降低 RDP $\varepsilon$），確保數據源頭的正確性。
