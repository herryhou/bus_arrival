...
fn test_velocity_exclusion_when_dist_exceeds_vmax() {
    ...
    let result = find_stop_index(5000, 1000, 1, &stops, 0, &None);
...
fn test_velocity_inclusion_when_dist_within_vmax() {
    ...
    let result = find_stop_index(s_cm, v_filtered, 1, &stops, 0, &None);
...
fn test_gps_recovery_with_realistic_elapsed_time() {
    ...
    let result = find_stop_index(s_cm, v_filtered, dt_since_last_fix, &stops, 0, &None);
    ...
    let result_quick = find_stop_index(s_cm, v_filtered, 1, &stops, 0, &None);
