...
fn test_gps_outside_bounds_returns_last_idx() {
    ...
    shared::binfile::pack_route_data(
        &nodes,
        &[],
        &grid,
        25.0,
    ...
    // Load route data
    let route_data = shared::binfile::RouteData::load(&buffer)
    ...
    assert_eq!(result, last_idx,
        "GPS outside bounds should return last_idx conservatively, but got {}", result);
}
...
fn test_gps_outside_y_bounds_returns_last_idx() {
    ...
    assert_eq!(result, last_idx,
        "GPS outside y bounds should return last_idx conservatively, but got {}", result);
}
...
fn test_gps_inside_bounds_works_normally() {
    ...
    assert!(result <= 1,
        "GPS inside bounds should find valid segment, but got {}", result);
}
