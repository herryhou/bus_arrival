...
/**
 * Tests for fallback tile scaling and alignment. Fallback tiles are lower zoom tiles used when
 * higher zoom tiles are unavailable. They must be scaled and aligned to match the viewport's zoom
 * level.
 */
class FallbackTileTest {
        ...
        /**
         * Calculate screen coordinates for a tile's corner. Returns the screen position of the
        ...
        /**
         * Calculate screen dimensions for a fallback tile. Returns Pair(width, height) in screen
         * pixels.
         */
        private fun calculateTileScreenDimensions(
                ...
                actualTileZ: Int, // Actual zoom level of tile we have
                scale: Float,
                baseZ: Int = 12 // Base zoom level for world coordinates
        ): Pair<Float, Float> {
        ...
        fun fallback_tile_covers_same_area_as_native_tile() {
                ...
                val (nativeWidth, nativeHeight) =
                        calculateTileScreenDimensions(tileZ, actualTileZ, scale)
                ...
                // The fallback Z=15 bitmap covers 4x4 Z=17 tiles, so position at any of those 16
                // Z=17 tiles
                ...
                val fallbackPosition =
                        calculateTileScreenPosition(
                                ...
                                tileY,
                                tileZ,
                                fallbackActualZ, // Use native tileX/Y with fallback Z
                                CENTER_LAT,
                                CENTER_LON,
                                scale,
                                offset,
                ...
                val (fallbackWidth, fallbackHeight) =
                        calculateTileScreenDimensions(tileZ, fallbackActualZ, scale)
        ...
        fun multiple_fallback_levels_align_correctly() {
                ...
                val fallback16Z = 16
                val fallback16Position =
                        calculateTileScreenPosition(
                                nativeTileX,
                                nativeTileY,
                                tileZ,
                                fallback16Z, // Use native tileX/Y with fallback Z
                                CENTER_LAT,
                                CENTER_LON,
                                scale,
                                offset,
                ...
                // Use requested Z=17 grid for positioning
                val fallback15Z = 15
                val fallback15Position =
                        calculateTileScreenPosition(
                                nativeTileX,
                                nativeTileY,
                                tileZ,
                                fallback15Z, // Use native tileX/Y with fallback Z
                                CENTER_LAT,
                                CENTER_LON,
                                scale,
                                offset,
        ...
        fun fallback_tiles_stitch_with_native_tiles() {
                ...
                val (nativeWidth, nativeHeight) =
                        calculateTileScreenDimensions(tileZ, nativeZ, scale)

                // Native tile right edge
                val nativeRightEdge = nativeTileX + 1
                val nativeRightPosition =
                        calculateTileScreenPosition(
                                nativeRightEdge,
                                nativeTileY,
                                tileZ,
                ...
                val fallbackZ = 15
                val (fallbackWidth, fallbackHeight) =
                        calculateTileScreenDimensions(tileZ, fallbackZ, scale)
                ...
                val fallbackPosition =
                        calculateTileScreenPosition(
                                ...
                                nativeTileY,
                                tileZ,
                                fallbackZ, // Use native tileX/Y with fallback Z
                                CENTER_LAT,
                                CENTER_LON,
                                scale,
                                offset,
        ...
        fun fallback_tile_dimensions_at_different_scales() {
                ...
                assertEquals(
                        TILE_SIZE * 2.0f.pow(baseZ - fallbackZ),
                        width1,
                        POSITION_TOLERANCE,
                ...
                assertEquals(
                        TILE_SIZE * 2.0f.pow(baseZ - fallbackZ) * 2f,
                        width2,
                        POSITION_TOLERANCE,
                ...
                assertEquals(
                        TILE_SIZE * 2.0f.pow(baseZ - fallbackZ) * 0.5f,
                        width05,
                        POSITION_TOLERANCE,
        ...
        fun fallback_tile_handles_edge_cases() {
                ...
                val minDiffPosition =
                        calculateTileScreenPosition(
                                ...
                                tileY,
                                tileZ,
                                minDiffZ, // Use native tileX/Y with fallback Z
                                CENTER_LAT,
                                CENTER_LON,
                                scale,
                                offset,
                ...
                // Verify position is calculated without errors (allow some tolerance for tile grid
                // quantization)
                assertEquals(
                        500f,
                        minDiffPosition.x,
                        21f, // Increased tolerance for tile grid rounding
                ...
                val extremeDiffZ = 10
                val (extremeWidth, extremeHeight) =
                        calculateTileScreenDimensions(tileZ, extremeDiffZ, scale)

                // Verify dimensions use baseZ formula: 2^(15-10) = 32
                val baseZ = 15
                val expectedScale = 2.0f.pow(baseZ - extremeDiffZ)
                assertEquals(
                        TILE_SIZE * expectedScale,
                        extremeWidth,
                        POSITION_TOLERANCE,
