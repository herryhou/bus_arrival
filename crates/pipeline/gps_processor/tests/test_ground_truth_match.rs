...
fn test_ground_truth_alignment() {
    ...
    // Load ground truth
    let gt_json =
        std::fs::read_to_string("/Users/herry/project/pico2w/bus_arrival/ground_truth.json")
            .expect("Failed to read ground_truth.json");
    let ground_truth: Vec<serde_json::Value> =
        serde_json::from_str(&gt_json).expect("Failed to parse ground_truth.json");
    ...
    for (i, gt_stop) in ground_truth.iter().enumerate() {
        ...
        if let Some(stop) = route_data.get_stop(stop_idx) {
            println!(
                "  Route Stop {}: progress_cm={}, corridor=[{}, {}]",
            ...
            if let Some(node) = route_data.get_node(seg_idx) {
                ...
                println!(
                    "  Difference: stop_progress - node_cum_dist = {} cm ({} m)",
                    diff,
                    diff / 100
                );
            }
                ...
                if let Ok(record) = serde_json::from_str::<serde_json::Value>(&json_str) {
                    ...
                    for i in 0..route_data.stop_count {
                        if let Some(stop) = route_data.get_stop(i) {
                            if s_cm >= stop.corridor_start_cm as i64
                                && s_cm <= stop.corridor_end_cm as i64
                            {
                                println!(
                                    "  -> In Stop {} corridor [{}, {}]",
