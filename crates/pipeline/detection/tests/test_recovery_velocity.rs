...
fn test_velocity_exclusion_when_dist_exceeds_vmax() {
    ...
    // Stop 1 (10000): dist 5000 > 3000 (V_MAX_CMS * 1) -> excluded (i32::MAX)
    // Stop 2 (12000): dist 7000 > 3000 -> excluded (i32::MAX)
...
fn test_velocity_inclusion_when_dist_within_vmax() {
    ...
    // Stop 1 (7000): ahead of the bus, dist 2000 < 3000 (V_MAX_CMS * 1) -> included, score 2000
...
/// At 108 km/h (V_MAX_CMS), the bus could travel up to 300m in 10 seconds,
...
fn test_gps_recovery_with_realistic_elapsed_time() {
    ...
    // With dt=10, max_reachable = V_MAX_CMS * 10 = 30000 cm = 300m
    // Stop 1 is 1000cm ahead (within 300m), so it should NOT be excluded
    ...
    // With dt=1 (old behavior), max_reachable = V_MAX_CMS * 1 = 3000 cm = 30m
    // Stop 1 is 1000cm ahead (still within 30m from s=10000), so included
